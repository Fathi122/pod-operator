...
const (
	configMapKey     = "searchKey"
	podNewLabel      = "newlabel.com"
	podNewLabelValue = "newlabelvalue"
)
...
func (r *PodReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// get configMap in same namespace with name equal podName
	...
	if err := r.Get(ctx, nameSpacedConfigMap, &configMap); err != nil {
		...
		if value, ok := configMap.Data[configMapKey]; ok {
			log.Info("Found Key: " + configMap.Data[configMapKey])
			...
			if podLabelValue, ok := pod.Labels[podNewLabel]; ok && podLabelValue == value {
				// If the label should not be set but is, remove it.
				delete(pod.Labels, podNewLabel)
